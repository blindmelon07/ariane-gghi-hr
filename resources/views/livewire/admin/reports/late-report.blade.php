            ...
            @forelse ($this->paginatedReport as $row)
                ...
                <div class="px-4 py-3">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-gray-800 dark:text-gray-100 truncate">{{ $row['name'] }}</p>
                    ...
                    <p class="text-xs font-semibold text-red-600 dark:text-red-400 mt-1">{{ $row['late_min'] }} min late ({{ number_format($row['late_min'] / 60, 2) }} hrs)</p>
                </div>
            @empty
                <div class="px-4 py-12 text-center text-gray-400 dark:text-gray-500 text-sm">No late records for the selected period.</div>
            @endforelse
        </div>
