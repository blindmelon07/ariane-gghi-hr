<div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <p class="text-sm text-gray-500 mt-1">{{ ucfirst(str_replace('_', ' ', Auth::user()->role)) }} · {{ now()->format('l, F j, Y') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-800" wire:loading.class="opacity-50">{{ $this->totalActiveEmployees }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            <div class="p-3 bg-green-100 rounded-lg">
                <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Present</p>
                <p class="text-2xl font-bold text-green-600">{{ $this->presentToday }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-red-600">{{ $this->absentToday }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            <div class="p-3 bg-yellow-100 rounded-lg">
                <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Late</p>
                <p class="text-2xl font-bold text-yellow-600">{{ $this->lateToday }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-purple-600">{{ $this->pendingLeaves }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500 uppercase tracking-wide">Pending Payroll</p>
                <p class="text-2xl font-bold text-blue-600">{{ $this->pendingPayroll }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Quick Actions</h4>
            <div class="space-y-2">
                <button wire:click="syncBioTime" class="w-full flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition">
                ...
                <a href="{{ route('admin.payroll') }}" wire:navigate class="w-full flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition">
                    <svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Process Payroll
                </a>
                <a href="{{ route('admin.reports.attendance') }}" wire:navigate class="w-full flex items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            @forelse ($this->todayBirthdays as $emp)
                <div class="flex items-center gap-3 py-2 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                    <div class="w-8 h-8 bg-pink-100 rounded-full flex items-center justify-center text-pink-600 text-xs font-bold">
        ...
        {{-- Recent Activity --}}
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Recent Activity</h4>
            <div class="space-y-3 max-h-64 overflow-y-auto">
                @forelse ($this->recentActivity as $log)
                    <div class="flex items-start gap-3 text-sm {{ !$loop->last ? 'pb-3 border-b border-gray-100' : '' }}">
