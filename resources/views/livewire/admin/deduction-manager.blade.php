<div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500">Manage deduction types and assign them to employees.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                            ...
                            @if ($d->deductionType)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                                    {{ match($d->deductionType->category) {
                                        ...
                                        'benefit' => 'bg-purple-100 text-purple-700',
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-400">No deductions found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data @click.self="$wire.set('showModal', false)">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6">
            ...
            <div class="space-y-4">
                ...
                {{-- Deduction Type --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Deduction Type</label>
                    <select wire:model.live="deductionTypeId" class="w-full rounded-lg border-gray-300 text-sm">
                    ...
                    @error('deductionTypeId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input wire:model="remainingBalance" type="number" step="0.01" min="0" placeholder="0.00"
                        class="w-full rounded-lg border-gray-300 text-sm" />
                    <p class="text-[11px] text-gray-400 mt-0.5">For loans: total remaining. For recurring (PhilHealth, Pag-IBIG): set to 0.</p>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data @click.self="$wire.set('showTypeModal', false)">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[80vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800">Deduction Types</h3>
                <button wire:click="$set('editTypeId', -1)" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition">
            ...
            @foreach ($categories as $catKey => $catLabel)
                ...
                <div class="mb-4">
                    <h4 class="text-xs font-semibold text-gray-500 uppercase mb-2">{{ $catLabel }}</h4>
                    <div class="space-y-1">
                        @foreach ($catTypes as $t)
                        <div class="flex items-center justify-between px-3 py-2 rounded-lg {{ $t->is_active ? 'bg-gray-50' : 'bg-red-50/50' }}">
                            <div>
                                <span class="text-sm font-medium {{ $t->is_active ? 'text-gray-800' : 'text-gray-400 line-through' }}">{{ $t->name }}</span>
                                <span class="text-xs text-gray-400 ml-2">{{ $t->code }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <button wire:click="openEditType({{ $t->id }})" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Edit</button>
                                <button wire:click="toggleTypeActive({{ $t->id }})" class="text-xs font-medium {{ $t->is_active ? 'text-red-500 hover:text-red-700' : 'text-green-600 hover:text-green-800' }}">
            ...
            <div class="flex justify-end mt-4">
                <button wire:click="$set('showTypeModal', false)" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">Close</button>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data @click.self="$wire.set('showTypeModal', false)">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4">{{ $editTypeId === -1 ? 'New' : 'Edit' }} Deduction Type</h3>

            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Code</label>
                    <input wire:model="typeCode" type="text" placeholder="e.g. SSS_LOAN" class="w-full rounded-lg border-gray-300 text-sm uppercase" maxlength="30" />
                    @error('typeCode') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Name</label>
                    <input wire:model="typeName" type="text" placeholder="e.g. SSS Salary Loan" class="w-full rounded-lg border-gray-300 text-sm" />
                    @error('typeName') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Category</label>
                    <select wire:model="typeCategory" class="w-full rounded-lg border-gray-300 text-sm">
                        <option value="government">Government</option>
                        <option value="loan">Loan</option>
                        <option value="benefit">Benefit</option>
                        <option value="other">Other</option>
                    </select>
                    @error('typeCategory') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="flex justify-end gap-3 mt-6">
                <button wire:click="$set('editTypeId', null)" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition">Back</button>
                <button wire:click="saveType" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition">
