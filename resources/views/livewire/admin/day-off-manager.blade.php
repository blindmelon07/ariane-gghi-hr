<div>
    @if (session('success'))
        <div class="mb-4 rounded-lg bg-green-50 p-4 text-green-700 text-sm" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-transition>
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h3 class="text-lg font-bold text-gray-800">Day Off Management</h3>
            <p class="text-sm text-gray-500">Assign rest days, holidays, and special day offs to employees.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div class="flex flex-wrap items-end gap-4">
            ...
            <div>
                ...
                <input wire:model.live="filterMonth" type="month" class="rounded-lg border-gray-300 text-sm" />
            </div>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2.5">
                            ...
                            <p class="text-xs text-gray-400 font-mono">{{ $off->employee->emp_code ?? '' }}</p>
                        </td>
                        <td class="px-4 py-2.5">{{ $off->date->format('M d, Y') }}</td>
                        <td class="px-4 py-2.5 text-gray-500">{{ $off->date->format('l') }}</td>
                        <td class="px-4 py-2.5">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                                {{ match($off->type) {
                                    'rest_day' => 'bg-blue-100 text-blue-700',
                            ...
                            @if ($off->is_recurring)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-700 ml-1">Recurring</span>
                            @endif
                        </td>
                        <td class="px-4 py-2.5 text-gray-500">{{ $off->description ?? '—' }}</td>
                        <td class="px-4 py-2.5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button wire:click="openEdit({{ $off->id }})" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Edit</button>
                                <button wire:click="delete({{ $off->id }})" wire:confirm="Remove this day off?" class="text-red-600 hover:text-red-800 text-xs font-medium">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-gray-400">No day offs found for this period.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data @click.self="$wire.set('showModal', false)">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="text-lg font-bold text-gray-800 mb-4">{{ $editId ? 'Edit' : 'Add' }} Day Off</h3>
            ...
            <div class="space-y-4">
                ...
                <div x-data="{ open: false }" @click.outside="open = false">
                    ...
                    <input wire:model.live.debounce.300ms="empSearch" @focus="open = true" @input="open = true" type="text" placeholder="Search employee…"
                        class="w-full rounded-lg border-gray-300 text-sm" {{ $editId ? 'disabled' : '' }} />
                    @if (!$editId)
                ...
                @if (!$editId)
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-2">Assignment Mode</label>
                    <div class="flex gap-2">
                        <button type="button" wire:click="$set('mode', 'single')"
                            class="flex-1 py-2 px-3 text-sm font-medium rounded-lg border transition
                            {{ $mode === 'single' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                            Single Date
                        </button>
                        <button type="button" wire:click="$set('mode', 'recurring')"
                            class="flex-1 py-2 px-3 text-sm font-medium rounded-lg border transition
                            {{ $mode === 'recurring' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                            Recurring Weekly
                        </button>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-2">Days of Week</label>
                    <div class="flex flex-wrap gap-2">
                        ...
                        <label class="flex items-center gap-1.5 px-3 py-2 rounded-lg border cursor-pointer text-sm transition
                            {{ in_array($i, $selectedDays) ? 'bg-indigo-50 border-indigo-400 text-indigo-700' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50' }}">
                            <input type="checkbox" wire:model.live="selectedDays" value="{{ $i }}" class="rounded border-gray-300 text-indigo-600 w-3.5 h-3.5" />
                            {{ $label }}
                        </label>
                        @endforeach
                    </div>
                    @error('selectedDays') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                {{-- Validity date range --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Validity Period</label>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <input wire:model="dateFrom" type="date" class="w-full rounded-lg border-gray-300 text-sm" />
                            <p class="text-[11px] text-gray-400 mt-0.5">From</p>
                            @error('dateFrom') <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <input wire:model="dateTo" type="date" class="w-full rounded-lg border-gray-300 text-sm" />
                            <p class="text-[11px] text-gray-400 mt-0.5">To</p>
                            @error('dateTo') <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="flex gap-2 mt-2">
                        <button type="button" wire:click="$set('dateTo', '{{ now()->addMonths(3)->format('Y-m-d') }}')"
                            class="text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200">3 months</button>
                        <button type="button" wire:click="$set('dateTo', '{{ now()->addMonths(6)->format('Y-m-d') }}')"
                            class="text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200">6 months</button>
                        <button type="button" wire:click="$set('dateTo', '{{ now()->endOfYear()->format('Y-m-d') }}')"
                            class="text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200">End of year</button>
                        <button type="button" wire:click="$set('dateTo', '{{ now()->addYear()->format('Y-m-d') }}')"
                            class="text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200">1 year</button>
                    </div>
                </div>
                @endif
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" x-data @click.self="$wire.set('showBulkModal', false)">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
            <h3 class="text-lg font-bold text-gray-800 mb-1">Bulk Assign Day Offs</h3>
            <p class="text-sm text-gray-500 mb-4">Assign recurring day offs to all active employees in a department.</p>

            <div class="space-y-4">
                {{-- Department --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Department</label>
                    <select wire:model="bulkDept" class="w-full rounded-lg border-gray-300 text-sm">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @foreach ($dayLabels as $i => $label)
                        <label class="flex items-center gap-1.5 px-3 py-2 rounded-lg border cursor-pointer text-sm transition
                            {{ in_array($i, $bulkSelectedDays) ? 'bg-indigo-50 border-indigo-400 text-indigo-700' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50' }}">
                            <input type="checkbox" wire:model.live="bulkSelectedDays" value="{{ $i }}" class="rounded border-gray-300 text-indigo-600 w-3.5 h-3.5" />
                            {{ $label }}
                        </label>
                        @endforeach
                    </div>
                    @error('bulkSelectedDays') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <input wire:model="bulkDateFrom" type="date" class="w-full rounded-lg border-gray-300 text-sm" />
                            <p class="text-[11px] text-gray-400 mt-0.5">From</p>
                            @error('bulkDateFrom') <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <input wire:model="bulkDateTo" type="date" class="w-full rounded-lg border-gray-300 text-sm" />
                            <p class="text-[11px] text-gray-400 mt-0.5">To</p>
                            @error('bulkDateTo') <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="flex gap-2 mt-2">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Type</label>
                    <select wire:model="bulkType" class="w-full rounded-lg border-gray-300 text-sm">
                        <option value="rest_day">Rest Day</option>
                        <option value="holiday">Holiday</option>
                        <option value="special">Special</option>
                        <option value="other">Other</option>
