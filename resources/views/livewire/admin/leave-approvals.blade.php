<div x-data="{ showModal: false }"
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-6">Leave Approvals</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
            ...
            <select wire:model.live="filterStatus" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <select wire:model.live="filterType" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <select wire:model.live="filterDept" class="rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($this->pendingRequests as $req)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-800">{{ $req->employee->full_name }}</div>
                                <div class="text-xs text-gray-400">{{ $req->employee->emp_code }}</div>
                            </td>
                            <td class="px-4 py-3 text-gray-600">{{ $req->leaveType->code }}</td>
                            <td class="px-4 py-3 text-gray-600 text-xs">
                                {{ $req->start_date->format('M d') }} – {{ $req->end_date->format('M d, Y') }}
                            </td>
                            <td class="px-4 py-3 text-gray-600">{{ $req->total_days }}</td>
                            <td class="px-4 py-3 text-gray-600 max-w-[180px] truncate">{{ $req->reason }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-gray-400">No leave requests found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <template x-teleport="body">
        <div x-show="showModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
