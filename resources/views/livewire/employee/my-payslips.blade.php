<div>
    ...
    @if ($this->payslips->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-12 text-center">
            ...
            <p class="mt-3 text-gray-500 dark:text-gray-400 text-sm">No payslips for {{ $year }}.</p>
        </div>
    @else
        <div class="space-y-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 $slip->payrollPeriod->start_date->format('M d') }} – {{ $slip->payrollPeriod->end_date->format('M d, Y') }}</p>
                        </div>
                    ...
                    <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                            <p class="text-gray-500 dark:text-gray-400 Pay</p>
                            <p class="font-medium text-gray-900 dark:text-gray-100">₱ {{ number_format($slip->basic_pay, 2) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-gray-400
                            <p class="font-medium text-gray-900 dark:text-gray-100">₱ {{ number_format($slip->overtime_pay, 2) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-gray-400 Pay</p>
                            <p class="font-medium text-gray-900 dark:text-gray-100">₱ {{ number_format($slip->gross_pay, 2) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-gray-400 Deductions</p>
                            <p class="font-medium text-red-600 dark:text-red-400">₱ {{ number_format($slip->total_deductions, 2) }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 border-t pt-3 flex justify-end">
                        <a href="{{ route('payslips.download', $slip->id) }}" class="inline-flex items-center gap-1 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium">
